...
def create_data_structure(string_input):
    ...
    lst_1 = string_input.split('.')
    lst_2 = []
    for i in xrange(0, len(lst_1) - 1, 2):
        lst_2.append([[lst_1[i]], [lst_1[i + 1]]])

    for i in lst_2:
        for e in i[0]:
            person, people = e.split(' is connected to ')
            network[person] = [people.split(', ')]
        for o in i[1]:
            person, games = o.split(' likes to play ')
            if person in network:
                network[person].append(games.split(', '))
...
def get_secondary_connections(network, user):
    if user not in network:
        ...
        if not network[user][0]:
            ...
            return list(set([secondary for primary in get_connections(network, user)
                             for secondary in get_connections(network, primary)]))
...
def find_path_to_friend(network, user_A, user_B, path=None):
    ...
    for connection in get_connections(network, user_A):
        if connection not in path:
            next_path = find_path_to_friend(network, connection, user_B, path)
            if next_path:
                return next_path
