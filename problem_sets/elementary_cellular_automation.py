...
def cellular_automaton(input_string, pattern_num, generations):
    n = len(input_string)
    for i in range(generations):
        new_string = ""
        for j in range(n):
            pattern = input_string[j - 1] + input_string[j] + input_string[(j + 1) % n]
            new_string += create_pattern(pattern_num)[pattern]
